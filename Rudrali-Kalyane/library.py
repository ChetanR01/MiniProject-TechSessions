...
def update_book():
    while True:
        ...
        query="UPDATE book SET title = %s, author = %s, quantity = %s WHERE book_id = %s" 
...
def issue_book():
    ...
    mycursor.execute("SELECT * FROM issued_books WHERE student_id = %s AND book_id = %s", (student_id, book_id))
...
def return_book():
    ...
    is_lost = input("Is the book lost? (Y/N): ").strip().lower()
